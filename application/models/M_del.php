...
class M_del extends CI_Model {

    /**
     * @name string TABLE_NAME Holds the name of the table in use by this model
     */
    const TABLE_NAME = 'invento_del';

    /**
     * @name string PRI_INDEX Holds the name of the tables' primary index used in this model
     */
    const PRI_INDEX = 'dl_id';
    ...
    public function update(Array $data, $where = array()) {
        ...
        public function getAll($where = null , $all = false)
        {
            ...
            if (!$all) {
                $this->db->where('dl.dl_id >', 0);
            }

            $this->db->join('status st', 'dl.st_id = st.sta_id', 'left');  


            $this->db->join('invento_items it', 'dl.it_id = it.it_id', 'left');

            // $this->db->join('invento_categories cat', 'dl.cat_id = cat.ct_id', 'left');

            $this->db->join('invento_users us', 'dl.us_user = us.id', 'left');

            $this->db->order_by('dl.dl_date', 'desc');
