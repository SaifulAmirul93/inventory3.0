<?php
	...
	class M_user extends CI_Model {
	     ...
	     */
	    const TABLE_NAME = 'invento_users';
	    ...
	    public function getAll($where = null , $all = false)
	    {
	        ...
	        if ($all) {
	        	$this->db->where_not_in('user.role', $all);
	        }
	                
	        $this->db->join('user_level ul', 'role = ul.ul_id', 'left');
	        $result = $this->db->get()->result();
	        if ($result) {
	            return $result;
	        } else {
	     ...
	     public function getAll2($where = null , $all = false)
	    {
	    	$this->db->select('*');
	        $this->db->from(self::TABLE_NAME);
	        if ($where !== NULL) {
